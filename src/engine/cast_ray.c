...
/*   Updated: 2022/01/17 23:22:03 by jekim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_distance_by_dda(t_intvec *map_index, t_intvec *step, t_ray *ray, int **map_matrix)
{
	int hit;
	int side;
	...
	while (!hit)
	{
		...
		if (map_matrix[map_index->x][map_index->y] == 2) hit = 1;
	}
}

void send_ray(t_player *plyr, t_window *win, int index, int **map)
{
	t_intvec map_idx;
	t_intvec step;
...
int cast_ray(t_window *win, t_player *plyr, int **map)
{
	int i;
